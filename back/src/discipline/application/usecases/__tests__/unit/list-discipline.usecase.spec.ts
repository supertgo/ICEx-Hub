...
describe('List disciplines use cases unit tests', () => {
  ...
  it('should return sorted by created at by default', async () => {
    ...
    const disciplines = [
      ...
      createDisciplineEntity({ createdAt: new Date(initialDate.getTime() + 1) }),
      createDisciplineEntity({ createdAt: new Date(initialDate.getTime() + 2) }),
    ];
  ...
  it('should return disciplines filtered, paginated and sorted', async () => {
    const disciplines = [
