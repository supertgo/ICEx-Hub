...
export const disciplineSeed = async (prisma: PrismaClient) => {
  for (const discipline of disciplinesSI) {
    const courseRecord = await prisma.course.findUnique({
      where: {
        code: discipline.courseCode,
      },
    });

    const coursePeriodRecord = await prisma.coursePeriod.findUnique({
      where: {
        unique_period_course: {
          courseId: courseRecord.id,
          name: `${discipline.periodo}º período`,
        },
      },
    });
    ...
    await prisma.discipline.upsert({
      where: {
        unique_discipline: {
          courseId: courseRecord.id,
          coursePeriodId: coursePeriodRecord.id,
